...
import logRoutes from './logs/routes/log.routes';
import userRoutes from './users/routes/user.routes';
import { handleDashboardStats } from './dashboard/dashboard.controller';
...
// ── Core middleware ───────────────────────────
app.use(helmet());
app.use(requestIdMiddleware); // Only once
...
app.get('/health', async (_req, res) => {
  ...
  try {
    ...
    const result = (await prisma.$queryRaw`SELECT count(*) as cnt FROM pg_stat_activity WHERE state = 'active'`) as Array<{ cnt: bigint }>;
  ...
  res.status(dbStatus === 'ok' ? 200 : 503).json({
    ...
    timestamp: new Date().toISOString(), uptime: Math.round(uptime),
    db: { status: dbStatus, activeConnections: dbConnectionCount, responseTimeMs: durationMs },
    memory: { heapUsedMB: Math.round(memUsage.heapUsed / 1024 / 1024), heapTotalMB: Math.round(memUsage.heapTotal / 1024 / 1024), rssMB: Math.round(memUsage.rss / 1024 / 1024) },
    environment: process.env.NODE_ENV || 'development',
  });
});

app.get('/uptime', (_req, res) => {
  res.status(200).json({ status: 'up', uptime: Math.round(process.uptime()), timestamp: new Date().toISOString() });
});
...
app.use('/api/v1/logs', apiLimiter, logRoutes);
app.use('/api/v1/users', apiLimiter, userRoutes);

// ── Dashboard stats ───────────────────────────
app.get('/api/v1/dashboard/stats', authenticate, authorize('WRITER', 'EDITOR', 'ADMIN'), handleDashboardStats);
