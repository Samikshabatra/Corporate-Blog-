...
export const handleCheckSlug = async (
  req: Request, res: Response, next: NextFunction,
): Promise<void> => {
  try {
    const { slug, excludeId } = req.query as { slug?: string; excludeId?: string };
    if (!slug || slug.trim().length < 3) {
      res.status(400).json({ status: 'error', message: 'Slug required (min 3 chars)' });
      return;
    }
    const existing = await prisma.post.findUnique({ where: { slug }, select: { id: true } });
    const unique = !existing || existing.id === excludeId;
    res.status(200).json({ status: 'success', unique, slug });
  } catch (err) { next(err); }
};
